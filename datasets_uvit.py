...
class FeatureDataset(Dataset):
    def __init__(self, path, split, n_extras, txt_feat=False):
        super().__init__()
        self.path = path
        self.split = split
        ...
        if txt_feat:
            self.data = np.load(f'{path}/{split}_img.npy')
            if os.getenv('COCO_SAMPLE_TXT', 'f') == 't':
                self.txt_data_path = f'{path}/txt_single'
                files_caption = glob.glob(os.path.join(self.txt_data_path, '*_*.npy'))
                n_captions = {k: 0 for k in range(len(self.data))}
                ...
                self.txt_data = np.load(f'{path}/{split}_txt.npy')
        else:
    ...
    def __getitem__(self, idx):
        if self.txt_feat:
            if os.getenv('COCO_SAMPLE_TXT', 'f') == 't':
                k = random.randint(0, self.n_captions[idx] - 1)
                txt_data = np.load(os.path.join(self.txt_data_path, f'{idx}_{k}.npy'))
                return self.data[idx], txt_data
            else:
                return self.data[idx], self.txt_data[idx]
        else:
...
class ImageNet256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder
    def __init__(self, path, cfg=False, p_uncond=None):
        ...
        self.test = FeatureDataset(path, split='val', n_extras=1)
...
class Churches256Features(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder
    def __init__(self, path, cfg=False, p_uncond=None, K=16):
        super().__init__()
        print('Prepare dataset...')
        self.train = FeatureDataset(path, split='train', n_extras=1)
        self.test = FeatureDataset(path, split='val', n_extras=1)
    ...
    def sample_label(self, n_samples, device):
        return torch.randint(0, self.K, (n_samples, 1), device=device)


class T2IFeatures(DatasetFactory):  # the moments calculated by Stable Diffusion image encoder
    def __init__(self, path, cfg=False, p_uncond=None, txt_feat=False):
        super().__init__()
        print('Prepare dataset...')
        self.train = FeatureDataset(path, split='train', n_extras=77, txt_feat=txt_feat)
        print('Prepare dataset ok')
        if txt_feat:
            self.empty_context = np.load(os.path.join(path, 'empty_context.npy'))
        else:
            self.empty_context = np.array([49406, 49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407,
                                           ...
                                           49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407,
                                           49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407, 49407,
                                           49407, 49407, 49407, 49407, 49407, 49407, 49407], dtype=np.longlong)
...
class MSCOCODatabase(Dataset):
    def __init__(self, root, annFile, size=None, centering=False):
    ...
    def __getitem__(self, index):
        ...
        # random choosing one caption
        # target = anns[random.randint(0, len(anns) - 1)]['caption']
...
def get_dataset(name, **kwargs):
    ...
    elif name == 'cc256_features' or name == 'mscoco256_features':
        return T2IFeatures(**kwargs)
    elif name == 'celeba':
